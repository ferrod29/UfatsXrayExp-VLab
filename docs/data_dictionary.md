...
`xas_model.xas_model`. Loader: `io.load_fit_params`; written by
`io.save_fit_params`.

This file is *derived*: it is regenerated by re-running the two
`fit_static_xas` calls in
[`../notebooks/06_static_and_transient_xas_modeling.ipynb`](../notebooks/06_static_and_transient_xas_modeling.ipynb).
Because it stores bare numbers it carries no record of the edge convention
it was fit under, and an earlier revision was fit against a logistic-sigmoid
edge rather than the `erf` edge of `lineshapes.erf_step` -- it still loaded
cleanly but modelled the data with a residual a third of the edge jump.
`tests/test_io.py::test_stored_fit_params_actually_reproduce_the_measured_spectra`
now pins the two together, so regenerate this file whenever the model
changes.
